...
function    renderTable(game) {
    ...
    buttonE2.innerHTML=`
    <button id='kaishi'>重开</button>`
    gameEl.append(buttonE2)
    buttonE2=gameEl.querySelector('button#kaishi');
    buttonE2.onclick=()=>{
        history.go(0)}
    ...
    number.innerHTML=`剩余雷的数量:${num(8,6)}`
    gameEl.append(number);
...
function renderWelcome(){
    ...
    <h id=over>天工出品</h>
    <img id=cnm src='https://pic.baike.soso.com/ugc/baikepic2/2140/20200702121209-1940449507_jpeg_606_546_38202.jpg/0'
    />
    ...
    buttonEl.onclick=()=>{    
        let game = new Game(8, 6, 0.5);
    game.shuffle();
    renderTable(game);}

    buttonEl=gameEl.querySelector('button#middle');
    buttonEl.onclick=()=>{    
        let game = new Game(24,12, 0.15);
    game.shuffle();
    renderTable(game);
